...
int main(int argc, const char* argv[]) {
  ...
  */
  ...
  thrust::host_vector<unsigned long long> h_i(N);
...
/*
  ...
  for (int row = 0; row < N; row++) {
      ...
      h_x[row] = std::atof(rec[1].c_str());
      h_y[row] = std::atof(rec[2].c_str());
      h_i[row] = std::stoull(rec[0]);
  }
*/
    ...
    for (const auto& rows : cells) {
      ...
      h_x[counter] = std::stof(rows[1]);
      h_y[counter] = std::stof(rows[2]);
      h_i[counter] = std::stoull(rows[0]);
  ...
  for(int i=0; i < N; i++)
  {
    std::bitset<32> bset_pair = std::bitset<32>(h_i[i]);
    ...
    std::string tmp_ipaddr = to_string(ip1_bset.to_ulong()) + "." + to_string(ip2_bset.to_ulong()) + "." + to_string(ip3_bset.to_ulong()) + "." + to_string(ip4_bset.to_ulong());
    
    std::cout << h_i[i] << "," << h_x[i] << "," << h_y[i] << ", cluster:" << h_clusterNo[i] << std::endl;
  }
